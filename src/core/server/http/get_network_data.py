...
async def get_network_data(avg_in_out=False) -> dict:
    ...
    it can also calculate the average network I/O statistics over a specified interval for each
    interface.

    Args:
        avg_in_out (bool): If True, calculates and includes average network I/O statistics over a
        specified interval.
            Defaults to False.

    Returns:
        dict: A dictionary containing network interfaces, their statistics, and optionally their
        average I/O statistics. The structure is as follows:
